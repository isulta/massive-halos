...
def profiles( part, Tmask=True, rbins=np.power(10, np.arange(np.log10(0.005258639741921723), np.log10(3), 0.05)), selectMainBranch=True, mainBranchHalfWidth=0.5, simname=None ):
    '''
    ...
    logprofiles = { k:[] for k in ['rho', 'T', 'Z', 'MachNumber', 'vrad', 'cs', 'tcool', 'nH', 'P_th', 'P_turb', 'e_CR', 'P_CR', 'K', 'K_Mweighted', 'Z_Mweighted', 'T_Mweighted', 'Pth_Mweighted', 'nH_Mweighted', 'P_turb_rad', 'tcoolshell', 'tcool_Mweighted', 'P_mag', 'CoolingRate', 'Z_Lweighted', 'T_mode', 'nH_mode'] }
    ...
    for r0,r1 in zip(rbins[:-1],rbins[1:]):
        ...
        if selectMainBranch:
            idx1 = idx[p0['Temperature'][idx] > 10**4.5]
            T_mode = continuous_mode(np.log10(p0['Temperature'][idx1])) #log space
            T_mode_lin = np.log10( continuous_mode(p0['Temperature'][idx1]) )
            logprofiles['T_mode'].append((T_mode, T_mode - mainBranchHalfWidth, T_mode + mainBranchHalfWidth, T_mode_lin))

            # idx2 = idx[p0['nH'][idx] < 10**-1.5]
            m, yinter_m12, yinter_m13 = -1.5, -4, -3
            yinter = yinter_m12 if 'm12' in simname else yinter_m13
            idx2 = idx[ p0['nH'][idx] < np.power(10, np.log10(p0['r_scaled'][idx])*m + yinter) ]
            nH_mode_log = continuous_mode(np.log10(p0['nH'][idx2])) #log space
            nH_mode = np.log10(continuous_mode(p0['nH'][idx2])) #linear space
            logprofiles['nH_mode'].append((nH_mode, nH_mode - mainBranchHalfWidth, nH_mode + mainBranchHalfWidth, nH_mode_log))
...
class Simulation:
    def __init__(self, simdir, snapnum, cachesim=False, find200c=False, satellitecut=False, calculateOutflows=False, datadir='simcachev2_Khist_outflow_selectMainBranch_0.5dex_T1e4.5cut_nHlimcut', maskToUse=None):
        self.simdir = simdir
        ...
        if cachesim:
            ...
            mask = True
            if satellitecut: mask = (self.part[0]['Temperature'] > 10**4.5)
            with np.errstate(divide='ignore', invalid='ignore'):
                self.pro = profiles(self.part, Tmask=mask, simname=self.simname)
