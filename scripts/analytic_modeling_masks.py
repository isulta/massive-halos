...
def profiles( part, Tmask=True, rbins=np.power(10, np.arange(np.log10(0.005258639741921723), np.log10(3), 0.05)) ):
    ...
    logprofiles = { k:[] for k in ['rho', 'T', 'Z', 'MachNumber', 'vrad', 'cs', 'tcool', 'nH', 'P_th', 'P_turb', 'e_CR', 'P_CR', 'K', 'K_Mweighted', 'Z_Mweighted', 'T_Mweighted', 'Pth_Mweighted', 'nH_Mweighted', 'P_turb_rad', 'tcoolshell', 'tcool_Mweighted', 'P_mag', 'CoolingRate', 'Z_Lweighted'] }
...
class Simulation:
    def __init__(self, simdir, snapnum, cachesim=False, find200c=False, satellitecut=False, calculateOutflows=False, datadir='simcachev2_Khist_outflow_allmasks_lim', maskToUse=None):
        self.simdir = simdir
        ...
        if cachesim:
            ...
            if satellitecut:
                gas_tree = cKDTree(self.part[0]['Coordinates'])
                star_tree = cKDTree(self.part[4]['Coordinates'])
                rhf = h5todict(os.path.join(self.simdir, f'halo/rockstar_dm/catalog_hdf5/halo_{self.snapnum:03}.hdf5'))
                try:
                    satmask = excise_satellites(rhf, gas_tree, star_tree, self.part[0]['posC'], self.part[0]['Rvir'])
                    satmask2 = excise_satellites(rhf, gas_tree, star_tree, self.part[0]['posC'], self.part[0]['Rvir'], Rmax_gas=1)
                    # mask = satmask&(self.part[0]['Temperature'] > 10**5.5)&(self.part[0]['nH'] < 10**(-2.5))
                    self.excisefraction = 1-np.sum(self.part[0]['Vi'][inrange(self.part[0]['r_scaled'], (0.,1))&satmask])/np.sum(self.part[0]['Vi'][inrange(self.part[0]['r_scaled'], (0.,1))])
                except:
                    satmask = True
                    satmask2 = True
                    self.excisefraction = -101
                print(self.simname, 'excise fraction', self.excisefraction)

                # yintern = -4 if 'm12' in self.simname else -3
                # yinterT = 4.2 if 'm12' in self.simname else 4.2+2/3
                # limnH = np.power(10, np.log10(self.part[0]['r_scaled'])*-1.5 + yintern)
                # limT = np.power(10, np.log10(self.part[0]['r_scaled'])*-1 + yinterT)
                def powerlaw(m, yinter_m12, yinter_m13):
                    yinter = yinter_m12 if 'm12' in self.simname else yinter_m13
                    return np.power(10, np.log10(self.part[0]['r_scaled'])*m + yinter)
                limT = powerlaw(-0.5, 5, 5.5)
                limnH = powerlaw(-1.5, -4, -3)
                lim2T = powerlaw(-0.15, 5.5, 5.5)
                masks = {
                    'all' : True,
                    'T1e4.5' : (self.part[0]['Temperature'] > 10**4.5),
                    'T1e5' : (self.part[0]['Temperature'] > 10**5.),
                    'T1e5.5' : (self.part[0]['Temperature'] > 10**5.5),
                    'T1e5_n1e-2' : (self.part[0]['Temperature'] > 10**5.)&(self.part[0]['nH'] < 10**(-2.)),
                    'T1e5.5_n1e-2.5' : (self.part[0]['Temperature'] > 10**5.5)&(self.part[0]['nH'] < 10**(-2.5)),
                    'exsat_T1e5.5_n1e-2.5' : satmask&(self.part[0]['Temperature'] > 10**5.5)&(self.part[0]['nH'] < 10**(-2.5)),
                    'exsat_T1e5_n1e-2' : satmask&(self.part[0]['Temperature'] > 10**5.)&(self.part[0]['nH'] < 10**(-2.)),
                    'exsat2_T1e5.5_n1e-2.5' : satmask2&(self.part[0]['Temperature'] > 10**5.5)&(self.part[0]['nH'] < 10**(-2.5)),
                    'lim' : (self.part[0]['Temperature'] > limT)&(self.part[0]['nH'] < limnH),
                    'exsat_lim' : satmask&(self.part[0]['Temperature'] > limT)&(self.part[0]['nH'] < limnH),
                    'lim2' : (self.part[0]['Temperature'] > lim2T)&(self.part[0]['nH'] < limnH),
                    'exsat_lim2' : satmask&(self.part[0]['Temperature'] > lim2T)&(self.part[0]['nH'] < limnH),
                }
            with np.errstate(divide='ignore', invalid='ignore'):
                self.pro = { k : profiles(self.part, Tmask=mask) for k,mask in masks.items() }
            ...
            self.Mr = res['Mr']
            if 'excisefraction' in res: self.excisefraction = res['excisefraction']
            if find200c:
            ...
            self.pro = res['pro']['all']
            Rmin = 0.1 * self.pro['Rvir'] #minimum radius for Mdot, Z avg in units pkpc
            self.Rcool = 0.3 * self.pro['Rvir'] #maximum radius, hardcoded and use 'all' mask for averaging
            ...
            # self.Rcool = self.pro['rmid'][self.pro['rmid']*self.pro['Rvir']>Rmin][np.flatnonzero(self.pro['tcoolshell'][self.pro['rmid']*self.pro['Rvir']>Rmin] > np.log10(self.tHubble))][0] * self.pro['Rvir'] #cooling radius in units pkpc 
            # if self.Rcool < Rmin: self.Rcool = 0.5 * self.pro['Rvir'] #raise Exception('Rcool not defined', self.simname)
            ...
            self.Z2Zsun = np.sum(10**self.pro['Z_Mweighted'][idxZavg] * self.pro['TotalMass:PartType0'][idxZavg]) / np.sum(self.pro['TotalMass:PartType0'][idxZavg]) #TODO Switch back to M weighted Z, look at TotalMass
            self.alpha_avg = np.mean(10**self.pro['P_turb'][idxZavg] / 10**self.pro['P_th'][idxZavg])
        
            self.potential = Potential_FIRE(self.Mr)
            self.pro = res['pro'][maskToUse]
        return
    ...
    def cacheeverything(self):
        ...
        res = {
            ...
            'Mr':           self.Mr,
            'excisefraction': self.excisefraction,
            #'R200c':        self.R200c,
            #'M200c':        self.M200c,
            **colormaps
        }
