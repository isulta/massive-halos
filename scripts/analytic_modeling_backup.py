...
def part_calculations(p, Rmax_Z=1, usetcoolWiersma=False, usetcoolWiersma_Zbins=True, Zbins=1000):
    ...
    p[0]['CoolingRateFIRE'] = p[0]['CoolingRate'].copy()
    if usetcoolWiersma:
...
class Potential_FIRE(CF.Potential):
    def __init__(self, Mr, Rmax=3, pro=None, fitrange=(0.04, 1.6)):
...
def profiles( part, Tmask=True, rbins=np.power(10, np.arange(np.log10(0.005258639741921723), np.log10(3), 0.05)), selectMainBranch=True, mainBranchHalfWidth=0.5 ):
    ...
    logprofiles = { k:[] for k in ['rho', 'T', 'Z', 'MachNumber', 'vrad', 'cs', 'tcool', 'nH', 'P_th', 'P_turb', 'e_CR', 'P_CR', 'K', 'K_Mweighted', 'Z_Mweighted', 'T_Mweighted', 'Pth_Mweighted', 'nH_Mweighted', 'P_turb_rad', 'tcoolshell', 'tcool_Mweighted', 'P_mag', 'CoolingRate', 'Z_Lweighted', 'T_mode', 'nH_mode', 'Mgas', 'Mdot', 'MdotIF', 'js'] }
...
class Simulation:
    def __init__(self, simdir, snapnum, cachesim=False, find200c=False, satellitecut=False, calculateOutflows=False, datadir='simcachev2_Khist_outflow_selectMainBranch_0.5dex_T1e5cut_nH_Mdot_modecut_MdotIF_jstests', fbranchcut=0.5, maskToUse=None):
        self.simdir = simdir
        ...
        if cachesim:
            ...
            self.potential = Potential_FIRE(self.Mr, pro=self.pro)
        return
